...
function classify(r){
  ...
  // La clasificación se mantiene flexible porque los valores exactos de Check/Validacion
  // pueden variar en la hoja. Se documenta aquí para ajustar las reglas sin tocar el diseño.
  let correct;
  if(["INCORRECTA","NO","0","FALSE","FALSO"].includes(valid)) correct=false;
  else if(["CORRECTA","SI","SÍ","1","TRUE","VERDADERO"].includes(valid)) correct=true;
  else if(["INCORRECTA","NO","0","FALSE","FALSO"].includes(check)) correct=false;
  else correct=promise;

  const effective = correct && (payments>0 || resta<=0);
...
function populateFilters(){
  const configs=[
    ["advisorFilter","Asesor"],["contactFilter","Tipo de contacto"],["supervisorFilter","SUPERVISOR"],["bannerFilter","Banner"]
  ];
...
function filtered(){
  const from=dateFrom.value,to=dateTo.value, advisor=key(advisorFilter.value),contact=key(contactFilter.value),sup=key(supervisorFilter.value),banner=key(bannerFilter.value);
  return rows.filter(r=>{
    ...
    if(banner&&key(first(r,"Banner"))!==banner)return false;
    return true;
  });
}
function aggregate(data, field){
  ...
  data.filter(r=>r._promise).forEach(r=>{
    ...
    const x=m.get(v);x.p++;r._correct?x.c++:x.i++;r._effective&&x.e++;x.a+=r._amount;
  });
...
function render(){
  ...
  kpiAmount.textContent=money(promises.reduce((s,r)=>s+r._amount,0));kpiPaid.textContent=money(promises.reduce((s,r)=>s+r._paid,0));
...
function renderDaily(data){
  const m=new Map();data.filter(r=>r._promise&&r._date).forEach(r=>{const d=isoDate(r._date);if(!m.has(d))m.set(d,{p:0,c:0,i:0,e:0,a:0});const x=m.get(d);x.p++;r._correct?x.c++:x.i++;r._effective&&x.e++;x.a+=r._amount});
...
async function load(){
  try{
    ...
    Papa.parse(CSV_URL,{download:true,header:true,skipEmptyLines:true,complete:res=>{
      rows=res.data.map(classify).filter(r=>r._date||r._promise);populateFilters();render();
      lastUpdate.textContent=new Date().toLocaleString("es-MX");status.textContent=`${rows.length.toLocaleString("es-MX")} registros cargados correctamente.`;
    },error:err=>{status.textContent="No fue posible leer Google Sheets. Revisa que la publicación CSV esté activa.";console.error(err)}});
  }catch(e){status.textContent="Error al cargar la información."}
}
document.querySelectorAll("select,input").forEach(e=>e.addEventListener("change",render));
resetBtn.addEventListener("click",()=>{dateFrom.value="";dateTo.value="";advisorFilter.value="";contactFilter.value="";supervisorFilter.value="";bannerFilter.value="";render()});
load();
