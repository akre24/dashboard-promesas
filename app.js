...
function detectMoraCol(fields){
  if(!fields)return;
  const found=fields.find(h=>{const k=key(h).replace(/\s+/g,"");return k.includes("MORA")&&k.includes("INICIAL")});
  if(found) MORA_COL=found;
}
...
function filtered(){
  ...
  return rows.filter(r=>{
    ...
    if(moras.length&&!moras.includes(key(first(r,MORA_COL))))return false;
    return true;
  });
}
